...
export async function POST(request: Request) {
  try {
    ...
    const fxRate = quote.currency === "USD" ? Number(env.USD_KRW_RATE) : 1;
    if (!Number.isFinite(fxRate) || fxRate <= 0) return Response.json({ error: "원달러 환율이 설정되지 않아 미국주식 주문을 중단했습니다." }, { status: 503 });
    ...
    const statements = [
      ...
      env.DB!.prepare(`INSERT INTO fills (id,order_id,participant_id,instrument_id,side,quantity_micros,price_micros,fee_krw,executed_at)
        SELECT ?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM orders WHERE id=?)`).bind(fillId, orderId, body.participantId, instrumentId, body.side, quantityMicros, nativePriceMicros, 0, now, orderId),
      ...
      env.DB!.prepare(`INSERT INTO quote_snapshots (instrument_id,price_micros,change_micros,change_rate_ppm,source,source_timestamp,received_at)
        SELECT ?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM fills WHERE id=?)
        ON CONFLICT(instrument_id) DO UPDATE SET price_micros=excluded.price_micros,change_micros=excluded.change_micros,change_rate_ppm=excluded.change_rate_ppm,source=excluded.source,source_timestamp=excluded.source_timestamp,received_at=excluded.received_at`)
        .bind(instrumentId, priceKrwMicros, Math.round(quote.change * fxRate * 1_000_000), Math.round(quote.changeRate * 10_000), quote.source, sourceTime, now, fillId),
    ];
