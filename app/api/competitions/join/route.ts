  ...
  try {
    ...
    if (existingCompetition) {
      return Response.json(
        { error: `이미 '${existingCompetition.name}' 대회에 참가 중입니다. 현재 대회에서 나간 뒤 다른 대회에 참가할 수 있습니다.` },
        { status: 409 },
      );
    }
    ...
    if (competition.status !== "active" || competition.endsAt <= now) return Response.json({ error: "참가할 수 없는 대회입니다." }, { status: 409 });
    ...
    await env.DB!.batch([
      ...
      ).bind(participantId, competition.id, user.id, competition.initialCashKrw, 0, now, user.id, now),
