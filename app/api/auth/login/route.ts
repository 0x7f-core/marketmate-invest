...
export async function POST(request: Request) {
  ...
  const user = await env.DB!.prepare(
    ...
    id: string; nickname: string; pinHash: string; pinSalt: string; failedLoginCount: number; lockedUntil: number;
  }>();
  const now = Date.now();
  if (!user || user.lockedUntil > now || !await verifyPin(pin, user.pinSalt, user.pinHash)) {
