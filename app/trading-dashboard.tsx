...
function DetailNavigation({ mobile = false }: { mobile?: boolean }) {
  const moveTo = (selector: string) => document.querySelector(selector)?.scrollIntoView({ behavior: "smooth", block: "start" });
  return <nav className={`np-detail-navigation${mobile ? " mobile" : " desktop"}`} aria-label="종목 상세 메뉴">
    <button className="active" onClick={() => moveTo(".np-chart")}>차트·시세</button>
    <button onClick={() => moveTo(".np-news")}>뉴스</button>
    <button onClick={() => moveTo(".np-stats")}>시세정보</button>
    <button onClick={() => moveTo(".order-panel")}>모의주문</button>
    <button onClick={() => moveTo(".np-portfolio-page")}>투자현황</button>
  </nav>;
}
