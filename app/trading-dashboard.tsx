...
function AccountSettingsDialog({ user, onUpdated, compact = false }: { user: User; onUpdated: (user: User) => void; compact?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="account-settings-dialog sm:max-w-md">
      ...
      <DialogFooter><Button onClick={save} disabled={busy || !canSave}>{busy ? "저장 중..." : "변경사항 저장"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
...
function RankingPanel({rows,participantId,onSelect}:{rows:LeaderboardRow[];participantId:string|null;onSelect:(row:LeaderboardRow)=>void}){return <section className="np-panel np-ranking"><div className="np-section-title"><h2>실시간 대회 순위</h2><span>{rows.length}명</span></div><div className="ranking-head"><span>순위·참가자</span><span>총자산</span><span>수익률</span></div>{rows.length?rows.map(row=>{const value=returnRate(row.totalAssetKrw,row.initialCashKrw);return <button className={row.participantId===participantId?"mine":""} onClick={()=>onSelect(row)} key={row.participantId}><b>{row.rank}</b><span><strong>{row.nickname}</strong><small>{displayRecentSymbols(row.recentSymbols)}</small></span><span>{formatKrw(row.totalAssetKrw)}</span><em className={value>=0?"up":"down"}>{value>=0?"+":""}{value.toFixed(2)}%</em><ChevronRight/></button>}):<p className="np-empty">체결된 거래가 1건 이상 있는 참가자부터 순위에 반영됩니다.</p>}</section>;}
