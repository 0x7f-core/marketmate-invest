...
export default function TradingDashboard(){
  ...
  const currentCompetition=competitions.find(item=>item.status==="active"&&item.endsAt>Date.now())??null;const activeCompetition=currentCompetition??competitions.find(item=>item.id===competitionId)??competitions[0]??null;const participantId=activeCompetition?.participantId??null;const myRank=leaderboard.find(row=>row.participantId===participantId);
