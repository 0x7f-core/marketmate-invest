...
import { ChevronRight, DoorOpen, Home, LineChart, LogOut, RefreshCw, Search, ShieldCheck, Star, Trash2, Trophy, WalletCards, X } from "lucide-react";
...
function LiveMarketStrip({quotes,onPick}:{quotes:MarketIndexQuote[];onPick:(market:Market)=>void}){const expected=[{id:"KOSPI",name:"코스피",market:"KR" as Market},{id:"KOSDAQ",name:"코스닥",market:"KR" as Market},{id:"SPX",name:"S&P 500",market:"US" as Market},{id:"COMP",name:"나스닥 종합",market:"US" as Market},{id:"BTC",name:"비트코인",market:"CRYPTO" as Market}];return <div className="live-market-strip">{expected.map(meta=>{const item=quotes.find(q=>q.id===meta.id);return <button key={meta.id} onClick={()=>onPick(meta.market)}><span>{meta.name}<small>{item?"네이버증권":"시세 확인 중"}</small></span><strong>{item?`${item.price.toLocaleString("ko-KR",{maximumFractionDigits:item.id==="BTC"?0:2})}${item.unit}`:"-"}</strong><em className={(item?.rate??0)>=0?"up":"down"}>{item?`${item.rate>=0?"+":""}${item.rate.toFixed(2)}%`:""}</em></button>})}</div>;}
function NewsPanel({items,title,loading=false}:{items:NewsItem[];title:string;loading?:boolean}){const sorted=[...items].sort((a,b)=>b.publishedAt-a.publishedAt);return <section className="np-panel np-news"><div className="np-section-title"><h2>{title}</h2><span>{sorted.length?`${sorted.length}건 · 최신순`:""}</span></div>{sorted.length?sorted.slice(0,10).map(item=><article key={`${item.link}:${item.publishedAt}`}><a href={item.link} target="_blank" rel="noreferrer">{item.title}</a><span>{item.source} · {relativeTime(item.publishedAt)}</span></article>):<p className="np-empty">{loading?"네이버증권 뉴스를 불러오는 중입니다.":"표시할 뉴스가 없습니다."}</p>}</section>;}
...
export default function TradingDashboard(){
  const[auth,setAuth]=useState<"loading"|User|null>("loading");const[view,setView]=useState<AppView>("home");const[market,setMarket]=useState<Market>("KR");const[selected,setSelected]=useState<Quote>(DEFAULTS.KR);const[quoteStatus,setQuoteStatus]=useState<"loading"|"live"|"unavailable">("loading");const[indices,setIndices]=useState<MarketIndexQuote[]>([]);const[competitions,setCompetitions]=useState<Competition[]>([]);const[competitionId,setCompetitionId]=useState<string|null>(null);const[leaderboard,setLeaderboard]=useState<LeaderboardRow[]>([]);const[portfolio,setPortfolio]=useState<Portfolio|null>(null);const[selectedParticipant,setSelectedParticipant]=useState<LeaderboardRow|null>(null);const[marketSession,setMarketSession]=useState<MarketSession>(LOADING_MARKET_SESSION);const[revision,setRevision]=useState(0);const[watchlist,setWatchlist]=useState<WatchlistItem[]>([]);const[newsItems,setNewsItems]=useState<NewsItem[]>([]);const[newsLoading,setNewsLoading]=useState(false);
  ...
  useEffect(()=>{if(!auth||auth==="loading"||view!=="market")return;let active=true;let timer:ReturnType<typeof setTimeout>|undefined;const schedule=(delay:number)=>{if(active)timer=setTimeout(load,Math.max(1_000,Math.min(120_000,delay)));};const load=()=>{setQuoteStatus(current=>current==="live"?current:"loading");fetch(`/api/quotes?market=${selected.market}&symbols=${encodeURIComponent(selected.symbol)}&exchange=${encodeURIComponent(selected.exchange)}`,{cache:"no-store"}).then(async r=>{const data=await r.json() as{quotes?:Array<{price:number;change:number;changeRate:number;currency:"KRW"|"USD";exchangeRate:number;pollingInterval?:number;venue?:"KRX"|"NXT";open?:number;high?:number;low?:number;volume?:number}>};if(!r.ok||!data.quotes?.[0])throw new Error();const q=data.quotes[0];if(active){setSelected(current=>current.market===selected.market&&current.symbol===selected.symbol?{...current,exchange:q.venue??current.exchange,price:q.price,change:q.change,rate:q.changeRate,currency:q.currency,exchangeRate:q.exchangeRate,open:q.open,high:q.high,low:q.low,volume:q.volume}:current);setQuoteStatus("live");schedule(q.pollingInterval??5_000);}}).catch(()=>{if(active){setQuoteStatus("unavailable");schedule(10_000);}});};void load();return()=>{active=false;if(timer)clearTimeout(timer);};},[auth,selected.market,selected.symbol,selected.exchange,view]);
  useEffect(()=>{if(!auth||auth==="loading"||!["home","market","news"].includes(view))return;const key=`${selected.market}:${selected.symbol}`;const cached=clientNewsCache.get(key);if(cached&&cached.expiresAt>Date.now()){setNewsItems(cached.items);setNewsLoading(false);return;}const controller=new AbortController();setNewsLoading(true);const params=new URLSearchParams({market:selected.market,name:selected.name,symbol:selected.symbol,exchange:selected.exchange});fetch(`/api/news?${params.toString()}`,{signal:controller.signal}).then(async r=>r.ok?await r.json() as{items:NewsItem[]}:null).then(data=>{const items=(data?.items??[]).sort((a,b)=>b.publishedAt-a.publishedAt);clientNewsCache.set(key,{items,expiresAt:Date.now()+90_000});setNewsItems(items);}).catch(()=>setNewsItems([])).finally(()=>{if(!controller.signal.aborted)setNewsLoading(false);});return()=>controller.abort();},[auth,selected.market,selected.symbol,selected.name,selected.exchange,view]);
  useEffect(()=>{if(!auth||auth==="loading"||view!=="market")return;let active=true;const load=()=>fetch(`/api/market-status?market=${market}`,{cache:"no-store"}).then(async r=>r.ok?await r.json() as MarketSession:null).then(value=>{if(active&&value)setMarketSession(value);}).catch(()=>undefined);void load();const timer=setInterval(load,30_000);return()=>{active=false;clearInterval(timer);};},[auth,market,view]);
  const chooseInstrument=(instrument:Instrument)=>{if(instrument.market!==market)setMarketSession(LOADING_MARKET_SESSION);setMarket(instrument.market);setSelected({...instrument,price:0,change:0,rate:0,exchangeRate:1});setView("market");window.scrollTo({top:0,behavior:"smooth"});};
  const changeMarket=(next:Market)=>{if(next!==market)setMarketSession(LOADING_MARKET_SESSION);setMarket(next);setSelected(DEFAULTS[next]);setView("market");window.scrollTo({top:0,behavior:"smooth"});};
  ...
  return <div className="marketmate-v2"><header className="np-desktop-header"><div className="np-head"><button className="np-brand" onClick={()=>setView("home")}><span>MM</span><b>마켓메이트</b></button><SearchBox onSelect={chooseInstrument}/><div className="np-head-actions">{auth.role==="admin"&&<AdminDialog/>}<JoinDialog onChanged={loadCompetitions}/><span>{auth.nickname}</span><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div></div><nav>{nav.map(([key,label])=><button key={key} className={view===key?"active":""} onClick={()=>setView(key)}>{label}</button>)}</nav><LiveMarketStrip quotes={indices} onPick={changeMarket}/></header><header className="np-mobile-header"><div><button className="np-brand" onClick={()=>setView("home")}><span>MM</span><b>마켓메이트</b></button><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div>{view==="market"&&<SearchBox onSelect={chooseInstrument}/>}<nav>{[["home","마켓"],["watchlist","관심"],["market","국내"],["market-us","미국"],["market-crypto","가상자산"],["competition","대회"],["portfolio","MY"]].map(([key,label])=><button key={key} className={((key===view&&key!=="market")||(key==="market"&&view==="market"&&market==="KR")||(key==="market-us"&&view==="market"&&market==="US")||(key==="market-crypto"&&view==="market"&&market==="CRYPTO"))?"active":""} onClick={()=>key==="market-us"?changeMarket("US"):key==="market-crypto"?changeMarket("CRYPTO"):key==="market"?changeMarket("KR"):setView(key as AppView)}>{label}</button>)}</nav></header>
    ...
    <ParticipantActivityDialog row={selectedParticipant} onClose={()=>setSelectedParticipant(null)}/>{auth.role==="admin"&&<div className="np-mobile-admin"><AdminDialog/></div>}<nav className="np-mobile-bottom">{[["home",Home,"홈"],["watchlist",Star,"관심"],["market",LineChart,"시세"],["competition",Trophy,"대회"],["portfolio",WalletCards,"MY"]].map(([key,Icon,label])=>{const I=Icon as typeof Home;return <button key={String(key)} className={view===key?"active":""} onClick={()=>setView(key as AppView)}><I/><span>{String(label)}</span></button>})}</nav></div>;
}
