...
export default function TradingDashboard(){
  ...
  const chooseInstrument=(instrument:Instrument)=>{setMarketSession(LOADING_MARKET_SESSION);setMarket(instrument.market);setSelected({...instrument,price:0,change:0,rate:0,exchangeRate:1});setView("market");window.scrollTo({top:0,behavior:"smooth"});};
  const changeMarket=(next:Market)=>{setMarketSession(LOADING_MARKET_SESSION);setMarket(next);setSelected(DEFAULTS[next]);setView("market");window.scrollTo({top:0,behavior:"smooth"});};
