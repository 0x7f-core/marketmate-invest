...
type Quote = Instrument & {
  ...
  open?: number; high?: number; low?: number; volume?: number; tradingValue?: number;
  high52Week?: number; low52Week?: number; tradingVenue?: DomesticVenue; availableVenues?: DomesticVenue[];
};
type Competition = {
...
function ParticipantActivityDialog({ row, onClose }: { row: LeaderboardRow | null; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(row)} onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="activity-dialog sm:max-w-2xl">
    ...
    {status ? <p className="activity-status">{status}</p> : <Tabs defaultValue="positions"><TabsList className="activity-tabs"><TabsTrigger value="positions">보유종목 {activity?.positions.length ?? 0}</TabsTrigger><TabsTrigger value="fills">체결내역 {activity?.fills.length ?? 0}</TabsTrigger></TabsList>
      ...
      <TabsContent value="fills" className="activity-list">{activity?.fills.length ? activity.fills.map(fill => <div key={fill.id}><span><b>{fill.name}</b><small>{formatDateTime(fill.executedAt)} · {formatQuantity(fill.quantityMicros)}{fill.market === "CRYPTO" ? "개" : "주"}</small></span><span><b className={fill.side === "buy" ? "up" : "down"}>{fill.side === "buy" ? "매수" : "매도"}</b><small className={fill.returnRate == null ? "" : fill.returnRate >= 0 ? "up" : "down"}>{formatKrw(fillValueKrw(fill))} · {fill.returnRateKind === "realized" ? "실현" : "현재"} {formatReturnRate(fill.returnRate)}</small></span></div>) : <p>아직 체결내역이 없습니다.</p>}</TabsContent>
    </Tabs>}
...
export default function TradingDashboard(){
  ...
  return <div className="marketmate-v2"><header className="np-desktop-header"><div className="np-head"><button className="np-brand" onClick={()=>setView("home")}><span>MM</span><b>마켓메이트</b></button><SearchBox onSelect={chooseInstrument}/><div className="np-head-actions">{auth.role==="admin"&&<AdminDialog/>}<JoinDialog onChanged={loadCompetitions}/><span>{auth.nickname}</span><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div></div><nav>{nav.map(([key,label])=><button key={key} className={view===key?"active":""} onClick={()=>key==="market"?openMarketView():setView(key)}>{label}</button>)}</nav><LiveMarketStrip quotes={indices} onPick={changeMarket}/></header><header className="np-mobile-header"><div><button className="np-brand" onClick={()=>setView("home")}><span>MM</span><b>마켓메이트</b></button><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div>{view==="market"&&<SearchBox onSelect={chooseInstrument}/>}</header>
    ...
    {view==="market"&&<main className="np-page np-trading"><section className="np-trading-main"><div className="np-market-tabs">{([["KR","국내"],["US","미국·글로벌"],["CRYPTO","가상자산"]] as const).map(([key,label])=><button className={market===key?"active":""} onClick={()=>changeMarket(key)} key={key}>{label}</button>)}</div><section className="np-panel np-quote"><div className="np-quote-head"><span className="stock-title"><InstrumentLogo key={`${selected.market}:${selected.symbol}`} instrument={selected} size="lg"/><span><small>{displaySymbol(selected.market,selected.symbol)} · {selected.exchange}</small><h1>{selected.name}<button className={selectedInWatchlist?"starred":""} onClick={toggleWatchlist} aria-label="관심종목"><Star fill={selectedInWatchlist?"currentColor":"none"}/></button></h1></span></span><span className="live-pill"><i/>{quoteStatus==="live"?"네이버 실시간":quoteStatus==="loading"?"확인 중":"시세 지연"}</span></div>{selected.market==="KR"&&<div className="np-domestic-venue-switch"><span>현재가·거래 기준</span><div><button type="button" className={domesticVenue==="KRX"?"active":""} onClick={()=>changeDomesticVenue("KRX")}>KRX</button><button type="button" className={domesticVenue==="NXT"?"active":""} disabled={Boolean(selected.availableVenues)&&!selected.availableVenues?.includes("NXT")} onClick={()=>changeDomesticVenue("NXT")}>NXT</button></div></div>}<div className="np-price"><strong>{formatPrice(selected)}</strong>{selected.price>0&&<span className={selected.rate>=0?"up":"down"}>{selected.change>=0?"▲":"▼"} {Math.abs(selected.change).toLocaleString()} ({selected.rate>=0?"+":""}{selected.rate.toFixed(2)}%)</span>}{selected.market==="KR"&&<small>{domesticVenue} 현재가</small>}</div><div className="np-stats"><span>시가<b>{formatQuoteMetricPrice(selected,selected.open)}</b></span><span>고가<b className="up">{formatQuoteMetricPrice(selected,selected.high)}</b></span><span>저가<b className="down">{formatQuoteMetricPrice(selected,selected.low)}</b></span><span>거래량<b>{formatQuoteVolume(selected.volume)}</b></span><span>52주 최고<b className="up">{formatQuoteMetricPrice(selected,selected.high52Week)}</b></span><span>52주 최저<b className="down">{formatQuoteMetricPrice(selected,selected.low52Week)}</b></span><span>거래대금<b>{formatQuoteTradingValue(selected)}</b></span></div><div className="np-chart"><MarketChart quote={selected}/></div></section><div className="np-mobile-order"><OrderPanel quote={selected} participantId={participantId} availableCashKrw={portfolio?.account.availableCashKrw??0} heldQuantityMicros={selectedHolding} session={marketSession} domesticVenue={domesticVenue} onDomesticVenueChange={changeDomesticVenue} onFilled={handleFilled}/></div><NewsPanel items={newsItems} title={`${selected.name} 관련 뉴스`} loading={newsLoading}/></section><aside><OrderPanel quote={selected} participantId={participantId} availableCashKrw={portfolio?.account.availableCashKrw??0} heldQuantityMicros={selectedHolding} session={marketSession} domesticVenue={domesticVenue} onDomesticVenueChange={changeDomesticVenue} onFilled={handleFilled}/></aside></main>}
