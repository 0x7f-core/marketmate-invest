...
import { ChevronDown, ChevronRight, ChevronUp, DoorOpen, Flame, Home, LineChart, LogOut, Newspaper, RefreshCw, Search, ShieldCheck, Star, Trash2, Trophy, WalletCards, X } from "lucide-react";
...
async function localizeUsQuoteInstrument(instrument: Instrument): Promise<Instrument> {
  if (instrument.market !== "US" || /[가-힣]/.test(instrument.name)) return instrument;
  ...
  try {
    ...
    if (/[가-힣]/.test(name)) usQuoteNameCache.set(cacheKey, name);
  ...
  return (
    <main className="auth-shell">
      <section className="auth-card">
          <div className="auth-brand"><img className="np-brand-icon" src="/favicon.svg" alt="" /><b>마켓메이트</b></div>
...
function ParticipantActivityDialog({ row, onClose }: { row: LeaderboardRow | null; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(row)} onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="activity-dialog sm:max-w-2xl">
    <DialogHeader><DialogTitle>{row?.nickname}님의 투자현황</DialogTitle><DialogDescription>{row ? `참가일 ${formatJoinDate(row.joinedAt)} · ` : ""}같은 대회 참가자에게 공개되는 보유종목과 모의체결 내역입니다.</DialogDescription></DialogHeader>
...
function AccountSettingsDialog({ user, onUpdated, onDeleted, compact = false }: { user: User; onUpdated: (user: User) => void; onDeleted: () => void; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/account", { cache: "no-store" })
      .then(async response => {
        ...
        if (!response.ok || !result.user) throw new Error(result.error ?? "계정 정보를 불러오지 못했습니다.");
        if (!active) return;
        setNickname(result.user.nickname);
        setNicknameLocked(Boolean(result.nicknameLocked));
        onUpdated(result.user);
      })
      .catch(error => active && setStatus(error instanceof Error ? error.message : "계정 정보를 불러오지 못했습니다."));
    return () => { active = false; };
  }, [open, onUpdated]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.user) {
        ...
        setStatus(result.error ?? "계정 정보를 변경하지 못했습니다.");
        return;
      }
      onUpdated(result.user);
      setNickname(result.user.nickname);
  ...
  const deleteAccount = async () => {
    ...
    if (currentPin.length !== 4) {
      setStatus("계정을 삭제하려면 현재 비밀번호 4자리를 입력해주세요.");
      return;
    }
    if (!window.confirm("계정을 삭제하면 참가 중인 대회 기록, 주문·체결·보유내역, 관심종목이 모두 삭제됩니다. 내가 개설한 대회도 함께 삭제되며 복구할 수 없습니다. 정말 계정을 삭제할까요?")) return;
    setBusy(true);
    setStatus("");
    try {
      const response = await fetch("/api/auth/account", {
      ...
      if (!response.ok || !result.ok) {
        setStatus(result.error ?? "계정을 삭제하지 못했습니다.");
        return;
      }
      setOpen(false);
      onDeleted();
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger asChild><button type="button" className={`account-settings-trigger${compact ? " compact" : ""}`} aria-label="계정 설정">계정 설정</button></DialogTrigger>
    <DialogContent className="account-settings-dialog sm:max-w-md">
      <DialogHeader>
        <DialogTitle>계정 설정</DialogTitle>
        <DialogDescription>닉네임과 숫자 비밀번호를 변경할 수 있습니다.</DialogDescription>
      </DialogHeader>
      <div className="account-settings-fields">
        ...
        {nicknameLocked && <p className="account-settings-lock">참가 중인 대회가 있어 닉네임을 변경할 수 없습니다. 대회에서 나가거나 대회가 종료된 뒤 변경할 수 있습니다.</p>}
        <div className="account-settings-divider" />
        <label>현재 비밀번호
          <Input value={currentPin} onChange={event => setCurrentPin(event.target.value.replace(/\D/g, "").slice(0, 4))} type="password" inputMode="numeric" autoComplete="current-password" placeholder="숫자 4자리" />
        </label>
        <label>새 비밀번호 <small>변경하지 않으려면 비워두세요.</small>
          <Input value={newPin} onChange={event => setNewPin(event.target.value.replace(/\D/g, "").slice(0, 4))} type="password" inputMode="numeric" autoComplete="new-password" placeholder="새 숫자 4자리" />
        </label>
        <label>새 비밀번호 확인
          <Input value={confirmPin} onChange={event => setConfirmPin(event.target.value.replace(/\D/g, "").slice(0, 4))} type="password" inputMode="numeric" autoComplete="new-password" placeholder="새 비밀번호 다시 입력" disabled={!newPin} />
        </label>
...
function LiveMarketStrip({quotes,onPick}:{quotes:MarketIndexQuote[];onPick:(id:string,market:Market)=>void}){
  const expected=[
    {id:"KOSPI",name:"코스피",market:"KR" as Market},
    {id:"KOSDAQ",name:"코스닥",market:"KR" as Market},
    {id:"SPX",name:"S&P 500",market:"US" as Market},
    {id:"COMP",name:"나스닥 종합",market:"US" as Market},
    {id:"BTC",name:"비트코인",market:"CRYPTO" as Market},
    {id:"USDKRW",name:"원/달러 환율",market:"US" as Market},
  ];
  return <div className="live-market-strip">{expected.map(meta=>{
    ...
    return <button key={meta.id} data-market-strip-usdkrw={isFx?"1":undefined} onClick={()=>onPick(meta.id,meta.market)}>
      <span>{meta.name}<small data-market-strip-usdkrw-source={isFx?"1":undefined}>{item?"네이버증권":"시세 확인 중"}</small></span>
...
function NewsPanel({items,title,loading=false,usSplit=false}:{items:NewsItem[];title:string;loading?:boolean;usSplit?:boolean}){
  ...
  return <section className="np-panel np-news">
    ...
    {usSplit&&<div className="us-news-tabs" role="tablist" aria-label="미국주식 뉴스 구분">
      <button type="button" role="tab" aria-selected={newsTab==="WORLD"} className={newsTab==="WORLD"?"active":""} onClick={()=>setNewsTab("WORLD")}>해외 뉴스</button>
      <button type="button" role="tab" aria-selected={newsTab==="LOCAL"} className={newsTab==="LOCAL"?"active":""} onClick={()=>setNewsTab("LOCAL")}>국내 뉴스</button>
    </div>}
    {sorted.length?sorted.slice(0,10).map(item=><article key={`${item.kind??"NEWS"}:${item.link}:${item.publishedAt}`}><a href={item.link} target="_blank" rel="noreferrer">{item.title}</a><span>{item.source} · {relativeTime(item.publishedAt)}</span></article>):<p className="np-empty">{loading?"네이버증권 뉴스를 불러오는 중입니다.":usSplit?(newsTab==="LOCAL"?"표시할 국내 뉴스가 없습니다.":"표시할 해외 뉴스가 없습니다."):"표시할 뉴스가 없습니다."}</p>}
  </section>;
}
...
function RankingPanel({rows,participantId,onSelect}:{rows:LeaderboardRow[];participantId:string|null;onSelect:(row:LeaderboardRow)=>void}){return <section className="np-panel np-ranking"><div className="np-section-title"><h2>실시간 대회 순위</h2><span>{rows.length}명</span></div><div className="ranking-head"><span>순위·참가자</span><span>총자산</span><span>수익률</span></div>{rows.length?rows.map(row=>{const value=returnRate(row.totalAssetKrw,row.initialCashKrw);const ranked=row.rank!==null;return <button className={row.participantId===participantId?"mine":""} onClick={()=>onSelect(row)} key={row.participantId}><b>{ranked?row.rank:""}</b><span><strong>{row.nickname}</strong><small>{displayRecentSymbols(row.recentSymbols)}</small></span><span>{formatKrw(row.totalAssetKrw)}</span><em className={value>=0?"up":"down"}>{value>=0?"+":""}{value.toFixed(2)}%</em><ChevronRight/></button>}):<p className="np-empty">대회를 만들거나 초대코드로 참가해주세요.</p>}</section>;}
...
function ScheduleRows({rows,minutes,prefix,forceClosed=false}:{rows:DomesticScheduleRow[];minutes:number|null;prefix:string;forceClosed?:boolean}){
  ...
  return <div className="schedule-rows-viewport" ref={viewportRef}>{rows.map(row=>{const current=forceClosed?row.label==="장 마감":minutes!==null&&isScheduleCurrent(row,minutes);return <div className={`domestic-schedule-row${current?" current":""}${current&&forceClosed?" closed-current":""}`} key={`${prefix}:${row.label}`}>
    <span className="domestic-session-name">{current&&<i className={`schedule-live-dot${forceClosed?" closed":""}`} aria-label="현재 시간대"/>}<b>{row.label}</b></span>
...
function DomesticTradingSchedule(){
  ...
  return <div className="domestic-schedule">
    <div className="domestic-schedule-now"><span className={`schedule-live-dot${forceClosed?" closed":""}`}/><span>{forceClosed?(isHoliday?"휴장일 · 장 마감":"주말 · 장 마감"):"한국시간 현재"}</span><strong>{now?.label??"--:--"}</strong></div>
...
function UsTradingSchedule(){
  ...
  return <div className="domestic-schedule us-schedule">
    ...
    <div className="domestic-schedule-now"><span className={`schedule-live-dot${forceClosed?" closed":""}`}/><span>{forceClosed?(isHoliday?"미국 휴장일 · 장 마감":"미국 주말 · 장 마감"):"한국시간 현재"}</span><strong>{state?.label??"--:--"}</strong></div>
...
export default function TradingDashboard(){
  ...
  const moveWatchlistItem=async(itemId:string,direction:-1|1)=>{if(watchlistOrderBusy)return;const current=watchlist;const index=current.findIndex(item=>item.id===itemId);const target=index+direction;if(index<0||target<0||target>=current.length)return;const next=[...current];[next[index],next[target]]=[next[target],next[index]];setWatchlist(next);setWatchlistOrderBusy(true);try{const response=await fetch("/api/watchlist",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({items:next.map(item=>item.id)})});if(!response.ok)throw new Error("watchlist reorder failed");}catch{setWatchlist(current);window.alert("관심 종목 순서를 저장하지 못했습니다.");}finally{setWatchlistOrderBusy(false);}};
  const leaveCompetition=async()=>{const target=currentCompetition??activeCompetition;if(!target||!window.confirm(`${target.name} 대회에서 나갈까요? 내 모의투자 기록이 삭제됩니다.`))return;const response=await fetch(`/api/competitions/leave?competitionId=${encodeURIComponent(target.id)}`,{method:"DELETE"});const result=await response.json() as{error?:string};if(!response.ok)return window.alert(result.error??"대회에서 나가지 못했습니다.");await loadCompetitions();setPortfolio(null);setLeaderboard([]);setTopPicks([]);};
  ...
  return <div className="marketmate-v2"><header className="np-desktop-header"><div className="np-head"><button className="np-brand" onClick={()=>setView("home")}><img className="np-brand-icon" src="/favicon.svg" alt="" /><b>마켓메이트</b></button><SearchBox onSelect={chooseInstrument}/><div className="np-head-actions">{auth.role==="admin"&&<AdminDialog/>}{!currentCompetition&&<JoinDialog onChanged={loadCompetitions}/>}<AccountSettingsDialog user={auth} onUpdated={setAuth} onDeleted={()=>setAuth(null)}/><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div></div><nav>{nav.map(([key,label])=><button key={key} className={view===key?"active":""} onClick={()=>key==="market"?openMarketView():setView(key)}>{label}</button>)}</nav><LiveMarketStrip quotes={indices} onPick={pickMarketOverviewById}/></header><header className="np-mobile-header"><div><button className="np-brand" onClick={()=>setView("home")}><img className="np-brand-icon" src="/favicon.svg" alt="" /><b>마켓메이트</b></button><div className="np-mobile-account-actions"><AccountSettingsDialog user={auth} onUpdated={setAuth} onDeleted={()=>setAuth(null)} compact/><button aria-label="로그아웃" onClick={logout}><LogOut/></button></div></div></header>
    ...
    {view==="news"&&<main className="np-page np-single"><NewsPanel items={newsItems} title={`${selected.name} 및 주요 시장 뉴스`} loading={newsLoading} usSplit={selected.market==="US"}/></main>}
